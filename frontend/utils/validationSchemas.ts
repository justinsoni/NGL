...
export const playerRegistrationSchema = yup.object().shape({
  ...
  // Medical & Fitness
  hasInjuryHistory: yup
    .boolean()
    .required('Please specify if you have any injury history'),

  injuryNature: yup
    .string()
    .when('hasInjuryHistory', {
      is: true,
      then: (schema) => schema.required('Please describe the nature of your injury').min(3, 'Description must be at least 3 characters').max(500, 'Description must not exceed 500 characters'),
  ...
  lastInjuryDate: yup
    ...
    .when('hasInjuryHistory', {
      is: true,
      then: (schema) => schema.required('Please select the date of your last injury'),
      otherwise: (schema) => schema.notRequired()
    }),

  fitnessStatus: yup
    .string()
    .required('Please select your current fitness status'),

  minimumSalary: yup
    .number()
    .transform((value) => (isNaN(value) ? undefined : value))
    .required('Minimum salary expectation is required')
